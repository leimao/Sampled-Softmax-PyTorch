...
#decoder = utils.LinearDecoder(args.nhid, ntokens)
encoder = utils.Word2VecEncoder(ntokens, args.emsize, args.dropout)
sampled_softmax = utils.SampledSoftmax(ntokens = ntokens, nsampled = args.softmax_nsampled, nhid = args.nhid, tied_weight = None)
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        #    p.data.add_(-lr, p.grad.data)
        #torch.nn.utilss.clip_grad_norm(decoder.parameters(), args.clip)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if not best_val_loss or val_loss < best_val_loss:
            with open(args.save, 'wb') as f:
                model_state = {'model': model, 'encoder': encoder, 'decoder': sampled_softmax}
                # torch.save(model, f)
...
sampled_softmax = model_saved['decoder']
